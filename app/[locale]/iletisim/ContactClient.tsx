...
export default function ContactClient() {
    ...
    const t_extra = useTranslations("contact_extra");
        ...
        <main className="min-h-screen bg-slate-950 font-sans text-slate-100 flex flex-col">
            ...
            <div className="container mx-auto px-4 py-16 md:py-20 flex flex-col lg:flex-row gap-12">
                ...
                <div className="lg:w-1/3 bg-slate-900 p-8 rounded-2xl border border-slate-800 h-fit">
                    <h3 className="text-2xl font-bold text-white mb-8 border-b border-slate-800 pb-4">{t_extra("contact_info")}</h3>
                ...
                <div className="lg:w-2/3 bg-slate-900/50 p-8 md:p-12 rounded-2xl border border-slate-800 backdrop-blur-sm relative">
                    <h3 className="text-2xl font-bold text-white mb-8">{t_extra("write_us")}</h3>

                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-slate-400 text-sm font-semibold mb-2" htmlFor="name">{t_extra("fullname")}</label>
                                <input
                                    type="text"
                                    id="name"
                                    required
                                    ...
                                    placeholder={t_extra("fullname_placeholder")}
                                    value={formData.name}
                                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                />
                            </div>
                            <div>
                                <label className="block text-slate-400 text-sm font-semibold mb-2" htmlFor="email">{t_extra("email")}</label>
                                <input
                                    type="email"
                                    id="email"
                                    required
                                    className="w-full bg-slate-950 border border-slate-700 text-white rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder:text-slate-600"
                                    placeholder={t_extra("email_placeholder")}
                        ...
                        <div>
                            <label className="block text-slate-400 text-sm font-semibold mb-2" htmlFor="message">{t_extra("message")}</label>
                            <textarea
                                id="message"
                                required
                                rows={6}
                                ...
                                placeholder={t_extra("message_placeholder")}
